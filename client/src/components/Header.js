...
const Header = (props) => {
  ...
  return (
    <header
      className={
        isBorderVisible ? "header-container scrolled" : "header-container"
      }
    >
      <NavLink
        to={`/diner/${props.accountId}`}
        className="logo-container"
        onClick={() => props.turnOffCuisineTypeMode()}
      >
        <img
          className="white-logo"
          src={logo}
          alt="Food Truck TrackR logo white"
        />
      </NavLink>
      ...
      {!locationEditMode && (
        <section className="header-section-one">
          <div
            className="location-sub-div"
            onClick={() => setLocationEditMode(true)}
          >
            <i class="fas fa-map-marker-alt"></i>
            <h3>
              {abbreviatedLocation.number} {abbreviatedLocation.street}
            </h3>
          </div>
        </section>
      )}
        ...
        <section className="header-section-one">
          ...
          >
            ...
            <div className="location-edit-buttons">
              <div>
                <button className="location-edit-clear" onClick={null}>
                  Clear
                </button>
              </div>
              <div>
                <button
                  ...
                  onClick={() => setLocationEditMode(false)}
                >
                  X
                </button>
              </div>
