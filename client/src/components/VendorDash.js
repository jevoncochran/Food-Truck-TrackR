...
const VendorDash = props => {
    ...
    useEffect(() => {
        if (newImage && truckToChange) {
            const uploadTask = storage.ref(`owner-uploaded-truck-pics/${newImage.name}`).put(newImage);
            uploadTask.on(
                "state_changed",
                snapshot => {
                ...
                >
                    <div key={truck.id}>
                        ...
                        <img 
                            ...
                            src={truck.image} alt="truck image" 
                            onClick={() => {
...
export default connect(mapStateToProps, { clientSignOut, changeTruckImg })(VendorDash);