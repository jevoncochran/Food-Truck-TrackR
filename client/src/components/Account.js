...
const Account = props => {
    ...
    useEffect(() => {
        if (newProfilePic) {
            const uploadTask = storage.ref(`diner-profile-pics/${newProfilePic.name}`).put(newProfilePic);
            uploadTask.on(
                "state_changed",
                snapshot => {
            ...
            <div className="account-div">
                <div className="account-bio">
                    ...
                    <div className="profile-pic-div">
                        ...
                        <div className="circle">
                            {<img className="profile-img" src={profilePicUrl} alt="profile image" style={{ height: '250px', width: '400px' }} onClick={inputClick} />}
                        </div>
                    </div>
                </div>
                ...
                <div className="labels-and-inputs">
                        <div className="grouping-div">
                            ...
                            <select className="display-and-input" onChange={handleCountryChange} defaultValue={props.country}>{country_list.map(x => (
                                <option key={x}>{x}</option>
                            ))}</select>
                        </div>
                        <div className="grouping-div">
                            <p className="input-label">Language</p>
                            <select className="display-and-input" onChange={handleLanguageChange} defaultValue={props.language}>{language_list.map(x => (
                                <option key={x}>{x}</option>
                            ))}</select>
                        </div>
                        <div className="grouping-div">
                ...
                <input className="change-pic-input" type="file" id="prof-pic-file" name="file" placeholder="test" onChange={handleFileChange} ref={input => inputFile = input} />                    
...
export default connect(mapStateToProps, { clientSignOut, editCountry, editLanguage, changeProfilePic })(Account);